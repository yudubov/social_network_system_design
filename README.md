...
HDD и SSD (nVME): будет очень много дисков из-за небольшой максимальной емкости (до 32/30 ТБ) Данные актуальные и лучше избежать сложную конфигурацию, если позволяют ресурсы. 
...
• Для СВЕЖИХ постов MEDIA выбираем 36 дисков SSD 68 ТБ 
...
HDD
1. Disks_for_capacity = 7,5 ПБ (7 500 ТБ) / 100 Тб = 75 дисков
2. Disks_for_throughput = 18 000 Мб/c / 100 Мб/c = 18 дисков 
3. Disks_for_iops = 6100 / 100 = 61 диск
4. Disks = max(75, 18, 61) = 75 диск (+ запас 25% = 18 дисков)
• Для СТАРЫХ постов MEDIA выбираем 93 диска HHD по 100 ТБ
• Для поддержания IOPS требуется 18 дисков, а у нас будет 93 диска. 93 диска точно покроют CPU операциий на разжатие. 
