...
• Лимиты 
1. Пользователь может писать не более 500 комментариев в день к другим постам

2. Пользователь может публиковать не более 20 своих постов в день

3. Пользователь может просматривать не более 2000 постов других пользователей в день

4. Пользователь может оценивать не более 500 постов в день

5. Пользователь может подписывать не более чем на 500 других пользователей в день
...
- принцип шардирования подсистем - шардирвоание не требуется, требуется только репликация (все данные каждой подсистемы вмещаются на один диск)
...
- RF (Replication Factor) - 2 (ресурсы ограничены)
- репликация master-slave (пишем в master, читаем из master и slave) нет требований по доступности из нескольких дата-центров
- принцип шардирования подсистем - Round-Robin равномерно по кластеру
